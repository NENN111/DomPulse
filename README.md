...
Житель работает в чате MAX, а оператор открывает веб-приложение «Проблемы и показатели» по кнопке бота. Приложение показывает очередь, общие проблемы и показатели дома на телефоне и компьютере.
...
4. Диспетчер открывает **«Проблемы и показатели» → «Очередь»**, отвечает жителю, назначает срочность и ведёт заявку по этапам: `новое → принято → в работе → ожидает проверки`.
...
Понадобятся Docker Engine с Compose и заполненный `.env`. Скопируйте `.env.example` в `.env`, укажите действующий `MAX_BOT_TOKEN` и непредсказуемый локальный секрет в `MAX_WEBHOOK_SECRET`. Значение `MAX_API_BASE` из примера подходит для обычного подключения. Секрет можно получить командой `python -c "import secrets; print(secrets.token_urlsafe(32))"`; результат внесите в `.env`. Для профиля `polling` публичный адрес и открытый входящий порт не нужны. Если бот уже запущен локальной командой `python -m app.polling`, остановите этот процесс перед запуском того же токена в Docker.
...
Команда запускает API и `max-polling`; они используют одну SQLite-базу в томе `dompulse-data`. Сам бот остаётся доступен пользователям **в MAX**, пока работает контейнер `max-polling` и есть соединение с MAX Bot API. Открывайте чат бота в приложении MAX или его веб-версии, а не только локальный Swagger.
...
MAX Bot API нельзя воспроизвести внутри Docker: он принимает сообщения пользователей и доставляет ответы бота. Для проверки нужны созданный и доступный в MAX бот, действующий токен этого бота, интернет-доступ контейнера к `https://platform-api2.max.ru` и два аккаунта MAX для одновременной проверки ролей жителя и диспетчера. После запуска проверьте `docker compose --profile polling ps`, затем отправьте боту `/start` в MAX. Если MAX недоступен или токен неверен, локальные API и SQLite могут работать, но основной пользовательский сценарий проверить нельзя. ЕСИА и Геокодер Яндекса для этого сценария не требуются: привязка к дому выполняется одноразовым кодом УК.

## Mini App оператора

Mini App обслуживается тем же FastAPI по пути `/miniapp`; интерфейс и API используют одну базу с ботом. В меню оператора три кнопки «Очередь дома», «Общие проблемы дома» и «Показатели дома» заменены одной кнопкой **«Проблемы и показатели»**. Внутри приложения доступны фильтры очереди, карточка обращения, ответ жителю, смена статуса и срочности, подтверждение общего инцидента, графики и числовые значения показателей. Объявления по-прежнему создаются через кнопку бота.

Для просмотра оформления без входа в MAX откройте `http://127.0.0.1:8080/miniapp?preview=1` после запуска API. Режим просмотра показывает только вымышленные данные и не сохраняет действия.

В настройках бота на платформе MAX для партнёров укажите URL мини-приложения `https://app.dom-pulse.ru/miniapp`, а в `.env` задайте `MAX_BOT_USERNAME`. В меню оператора кнопка **«Проблемы и показатели»** открывает Mini App внутри MAX. MAX передаёт подписанные данные запуска, сервер проверяет подпись токеном бота и роль привязанного оператора; код вручную вводить не требуется. При каждой операции сервер заново проверяет роль и доступ к дому.

При открытии публичной HTTPS-ссылки во внешнем браузере без кнопки бота данных запуска MAX нет, поэтому доступ к данным закрыт. Оператор открывает Mini App через кнопку в чате бота или системную кнопку MAX.

Для временной проверки предусмотрен отдельный вход `python -m app.public_miniapp` на `127.0.0.1:8081`. Он отдаёт страницу, файлы и необходимые API Mini App; остальные маршруты, включая `/docs`, `/health` и webhook, закрыты. Данные доступны по проверенной подписи MAX; Bearer-токены через этот вход не принимаются. Туннель направляют на `http://127.0.0.1:8081`, а его адрес с путём `/miniapp` указывают в настройках бота MAX. Для MVP с постоянной доступностью нужен стабильный HTTPS-домен.
...
4. Диспетчер привязывает профиль кодом УК и нажимает **«Проблемы и показатели»** в чате бота. Mini App открывается внутри MAX сразу с очередью и показателями дома, без отдельного кода входа.
...
8. Для проверки общего инцидента создайте похожие обращения от двух жителей, затем подтвердите найденное совпадение во вкладке **«Общие проблемы»** Mini App.

Ожидаемый результат: новая заявка появляется в очереди своего дома; действия диспетчера и его ответ видны жителю; после отметки о выполнении житель подтверждает результат или возвращает заявку в работу. В «Показателях дома» диспетчер видит число активных и аварийных заявок, нарушения срока первой реакции, среднее время ответа, долю ответов в пределах SLA, частые категории и проблемные места. До появления подходящих заявок значения времени и SLA показываются как «ещё нет данных». Опубликованное диспетчером объявление появляется у жителей этого дома и в блоке последних объявлений показателей. При создании похожих обращений бот предлагает общий сигнал; объединение выполняется только после решения диспетчера.
...
| `MAX_BOT_USERNAME` | для Mini App | публичное имя того же бота из `GET /me`, без символа `@`; передаётся в кнопке `open_app` |
| `MINIAPP_PUBLIC_URL` | необязательно | публичный HTTPS-адрес страницы `/miniapp`; URL Mini App должен быть указан в настройках бота MAX |
...
- Московские адреса сравниваются без учёта регистра, пунктуации, слова «дом» и необязательного префикса «Москва». В списках они показываются как «Москва, Название улицы, дом N». При запуске существующие подписи домов приводятся к этому виду без изменения ID дома и связанных заявок. Для адреса в другом городе город нужно указывать явно.
...
- Для встроенного Mini App нужен публичный HTTPS-адрес, сохранённый в настройках бота на платформе MAX для партнёров. При открытии через MAX оператор входит автоматически по проверенной подписи запуска.
...
## Постоянный HTTPS-адрес Mini App

Для стабильной ссылки без собственного домена используйте закреплённый HTTPS-домен ngrok. Нужны аккаунт ngrok, authtoken и домен, назначенный аккаунту. Установите ngrok и настройте его локально:

```powershell
ngrok config add-authtoken <ваш-authtoken>
ngrok http 8081 --url https://<домен-из-ngrok>
```

Токен храните в настройках ngrok и не добавляйте в репозиторий. Публичный вход `app.public_miniapp` должен работать на `127.0.0.1:8081`. Укажите `https://<домен-из-ngrok>/miniapp` в настройках бота MAX и проверьте встроенную кнопку «Проблемы и показатели». Домен сохраняется после повторного подключения ngrok. Для непрерывной доступности должны работать компьютер, Mini App и ngrok; на бесплатном плане ngrok есть лимиты запросов и трафика.

### Домен через Cloudflare Tunnel

Для постоянной ссылки на собственном домене добавьте зарегистрированный домен в Cloudflare и замените его NS у регистратора на два NS, выданных Cloudflare. Дождитесь статуса Active. В Cloudflare откройте Networking → Tunnels, создайте туннель и добавьте Published application: hostname `app.<домен>`, Service URL `http://127.0.0.1:8081`. На машине с Mini App сохраните токен туннеля в игнорируемый Git файл `.local/cloudflare-token.txt` и запустите `cloudflared tunnel run --token-file .local/cloudflare-token.txt`. Не добавляйте токен в репозиторий. Укажите `https://app.<домен>/miniapp` в настройках бота MAX и проверьте встроенную кнопку. Для круглосуточной доступности необходимы работающие Mini App и туннель на постоянно включённом компьютере или сервере.

Для локального запуска в Windows используйте `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\start-local-windows.ps1`. Скрипт запускает бот, API, публичный Mini App и Cloudflare Tunnel, пропуская уже работающие процессы. Для автоматического запуска после входа в Windows один раз выполните `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\install-autostart-windows.ps1`. Задача планировщика называется `DomPulseLocal`; она не требует прав администратора и запускается после входа пользователя. Проверка: `Get-ScheduledTaskInfo -TaskName DomPulseLocal`. Для ручного повторного запуска: `Start-ScheduledTask -TaskName DomPulseLocal`. После выключения ПК или до входа пользователя сайт и бот недоступны.
